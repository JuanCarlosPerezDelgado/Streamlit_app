...
def representacion_grafico(propiedades_ciclo,variables_ciclo,indicadores_termodinamicos,velocidad_reproduccion,tipo_grafico_diferido,lista_variables_diferido,df_analizador_redes_ensayo,df_equipo_ensayo,df_evaporador_04_ensayo,df_valvula_expansion_05_ensayo,df_evaporador_06_ensayo,df_valvula_expansion_07_ensayo):
    lista_indicadores=['COP','Diagramas P-H y T-S','Potencias del ciclo','Flujos másicos','Rendimientos del compresor','Caracterización de los evaporadores y gas-cooler']
    ...
    if (set_indicadores_termodinamicos & set_lista_indicadores) or (variables_ciclo and propiedades_ciclo):
    ...
    if 'Tabla' in tipo_grafico_diferido and lista_variables_diferido:
        st.markdown("<br>", unsafe_allow_html=True) 
        #Información
        st.markdown("<p style='font-size: 17px;'><strong>Tabla de variables</strong></p>", unsafe_allow_html=True)
    ...
    if 'Scada' in tipo_grafico_diferido and lista_variables_diferido:
        ...
        'Rele compresor (on/off)': (1190,530),
    ...
    if 'Diagramas P-H y T-S' in indicadores_termodinamicos and 'Tabla' in tipo_grafico_diferido:
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("<p style='font-size: 17px;'><strong>Tabla de propiedades termodinámicas del ciclo</strong></p>", unsafe_allow_html=True)
    ...
    if 'COP' in indicadores_termodinamicos and 'Tabla' in tipo_grafico_diferido:
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("<p style='font-size: 17px;'><strong>Tabla de coeficientes de rendimiento del ciclo</strong></p>", unsafe_allow_html=True)
    ...
    if 'Potencias del ciclo' in indicadores_termodinamicos and 'Tabla' in tipo_grafico_diferido:
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("<p style='font-size: 17px;'><strong>Tabla de potencias del ciclo</strong></p>", unsafe_allow_html=True)
    ...
    if 'Rendimientos del compresor' in indicadores_termodinamicos and 'Tabla' in tipo_grafico_diferido:
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("<p style='font-size: 17px;'><strong>Tabla de rendimientos del compresor</strong></p>", unsafe_allow_html=True)
    ...
    if 'Caracterización de los evaporadores y gas-cooler' in indicadores_termodinamicos and 'Gráfico de líneas' in tipo_grafico_diferido:
        st.markdown("<br>", unsafe_allow_html=True)
        grafica_efectividad=st.empty()
        grafica_saltos=st.empty()

    if 'Caracterización de los evaporadores y gas-cooler' in indicadores_termodinamicos and 'Tabla' in tipo_grafico_diferido:
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("<p style='font-size: 17px;'><strong>Tabla de efectividad y saltos térmicos en evaporadores y gas-cooler</strong></p>", unsafe_allow_html=True)
        tabla_placeholder_evaporadores = st.empty()  # Placeholder para la tabla
        # Inicializar un DataFrame vacío para la tabla
        tabla_datos_evaporadores = pd.DataFrame(columns=['Efectividad (evaporador 1) (%)','Efectividad (evaporador 2) (%)','Salto térmico (entrada - evaporador 1) (K)','Salto térmico (salida - evaporador 1) (K)','Salto térmico (entrada - evaporador 2) (K)','Salto térmico (salida - evaporador 2) (K)','Salto térmico (salida - gass-cooler) (K)'])
        tabla_datos_evaporadores.index.name = 'Hora' 


    if variables_ciclo and propiedades_ciclo and ('Gráfico de líneas' in tipo_grafico_diferido):
        st.markdown("<br>", unsafe_allow_html=True)
        grafica_variables=st.empty()
        lista_propiedades_puntos=[]
        for punto in variables_ciclo:
            for propiedad in propiedades_ciclo:
                lista_propiedades_puntos.append(f'{propiedad} - Punto {punto}')

        lista_completa=['Temperatura (°C) - Punto 1', 'Presión (bar) - Punto 1', 'Entalpía (kJ/Kg) - Punto 1', 'Entropía (kJ/Kg·K) - Punto 1', 'Título de vapor - Punto 1', 'Densidad (Kg/m^3) - Punto 1', 'Caudal másico (Kg/s) - Punto 1',
                        'Temperatura (°C) - Punto 2', 'Presión (bar) - Punto 2', 'Entalpía (kJ/Kg) - Punto 2', 'Entropía (kJ/Kg·K) - Punto 2', 'Título de vapor - Punto 2', 'Densidad (Kg/m^3) - Punto 2', 'Caudal másico (Kg/s) - Punto 2',
                        'Temperatura (°C) - Punto 3', 'Presión (bar) - Punto 3', 'Entalpía (kJ/Kg) - Punto 3', 'Entropía (kJ/Kg·K) - Punto 3', 'Título de vapor - Punto 3', 'Densidad (Kg/m^3) - Punto 3', 'Caudal másico (Kg/s) - Punto 3',                     
                        'Temperatura (°C) - Punto 4s', 'Presión (bar) - Punto 4s', 'Entalpía (kJ/Kg) - Punto 4s', 'Entropía (kJ/Kg·K) - Punto 4s', 'Título de vapor - Punto 4s', 'Densidad (Kg/m^3) - Punto 4s', 'Caudal másico (Kg/s) - Punto 4s',         
                        'Temperatura (°C) - Punto 4', 'Presión (bar) - Punto 4', 'Entalpía (kJ/Kg) - Punto 4', 'Entropía (kJ/Kg·K) - Punto 4', 'Título de vapor - Punto 4', 'Densidad (Kg/m^3) - Punto 4', 'Caudal másico (Kg/s) - Punto 4',          
                        'Temperatura (°C) - Punto 5', 'Presión (bar) - Punto 5', 'Entalpía (kJ/Kg) - Punto 5', 'Entropía (kJ/Kg·K) - Punto 5', 'Título de vapor - Punto 5', 'Densidad (Kg/m^3) - Punto 5', 'Caudal másico (Kg/s) - Punto 5',
                        'Temperatura (°C) - Punto 6s', 'Presión (bar) - Punto 6s', 'Entalpía (kJ/Kg) - Punto 6s', 'Entropía (kJ/Kg·K) - Punto 6s', 'Título de vapor - Punto 6s', 'Densidad (Kg/m^3) - Punto 6s', 'Caudal másico (Kg/s) - Punto 6s',    
                        'Temperatura (°C) - Punto 6', 'Presión (bar) - Punto 6', 'Entalpía (kJ/Kg) - Punto 6', 'Entropía (kJ/Kg·K) - Punto 6', 'Título de vapor - Punto 6', 'Densidad (Kg/m^3) - Punto 6', 'Caudal másico (Kg/s) - Punto 6',
                        'Temperatura (°C) - Punto 7', 'Presión (bar) - Punto 7', 'Entalpía (kJ/Kg) - Punto 7', 'Entropía (kJ/Kg·K) - Punto 7', 'Título de vapor - Punto 7', 'Densidad (Kg/m^3) - Punto 7', 'Caudal másico (Kg/s) - Punto 7',
                        'Temperatura (°C) - Punto 8', 'Presión (bar) - Punto 8', 'Entalpía (kJ/Kg) - Punto 8', 'Entropía (kJ/Kg·K) - Punto 8', 'Título de vapor - Punto 8', 'Densidad (Kg/m^3) - Punto 8', 'Caudal másico (Kg/s) - Punto 8',
                        'Temperatura (°C) - Punto 9', 'Presión (bar) - Punto 9', 'Entalpía (kJ/Kg) - Punto 9', 'Entropía (kJ/Kg·K) - Punto 9', 'Título de vapor - Punto 9', 'Densidad (Kg/m^3) - Punto 9', 'Caudal másico (Kg/s) - Punto 9',
                        'Temperatura (°C) - Punto 10', 'Presión (bar) - Punto 10', 'Entalpía (kJ/Kg) - Punto 10', 'Entropía (kJ/Kg·K) - Punto 10', 'Título de vapor - Punto 10', 'Densidad (Kg/m^3) - Punto 10', 'Caudal másico (Kg/s) - Punto 10',
                        'Temperatura (°C) - Punto 11', 'Presión (bar) - Punto 11', 'Entalpía (kJ/Kg) - Punto 11', 'Entropía (kJ/Kg·K) - Punto 11', 'Título de vapor - Punto 11', 'Densidad (Kg/m^3) - Punto 11', 'Caudal másico (Kg/s) - Punto 11',
                        'Temperatura (°C) - Punto 12', 'Presión (bar) - Punto 12', 'Entalpía (kJ/Kg) - Punto 12', 'Entropía (kJ/Kg·K) - Punto 12', 'Título de vapor - Punto 12', 'Densidad (Kg/m^3) - Punto 12', 'Caudal másico (Kg/s) - Punto 12',
                        'Temperatura (°C) - Punto 13', 'Presión (bar) - Punto 13', 'Entalpía (kJ/Kg) - Punto 13', 'Entropía (kJ/Kg·K) - Punto 13', 'Título de vapor - Punto 13', 'Densidad (Kg/m^3) - Punto 13', 'Caudal másico (Kg/s) - Punto 13',
                        'Temperatura (°C) - Punto 14', 'Presión (bar) - Punto 14', 'Entalpía (kJ/Kg) - Punto 14', 'Entropía (kJ/Kg·K) - Punto 14', 'Título de vapor - Punto 14', 'Densidad (Kg/m^3) - Punto 14', 'Caudal másico (Kg/s) - Punto 14']


        tabla_acumulada = pd.DataFrame(columns=lista_completa)
        #tabla_acumulada.index.name = 'Hora'
        tabla_acumulada.loc[:,"Tiempo"] = range(len(df_ampliado_analogico))
        tabla_acumulada.loc[:,"Indice"] = df_ampliado_analogico.index.strftime('%H:%M')
        lista_propiedades_puntos=lista_propiedades_puntos+['Tiempo','Indice']

    
    if variables_ciclo and propiedades_ciclo and ('Tabla' in tipo_grafico_diferido):
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("<p style='font-size: 17px;'><strong>Tabla de propiedades termodinámicas de los puntos del ciclo</strong></p>", unsafe_allow_html=True)
        tabla_placeholder_propiedades_puntos = st.empty()  # Placeholder para la tabla
...
#
            ...
            if 'Potencias del ciclo' in indicadores_termodinamicos and 'Gráfico de líneas' in tipo_grafico_diferido:
                ...
                if media.loc[i,'Rele compresor (on/off)']==0:
                    ...
                    media.loc[i,'Consumo eléctrico (compresor) (kW)']=0
                    media.loc[i,'Potencia frigorífica (evaporadores) (kW)']=0
                    media.loc[i,'Potencia ideal (compresor) (kW)']=0
                    media.loc[i,'Potencia cedida (gas-cooler) (kW)']=0        
                ...
                fig_potencias= px.line(
                    ...
                    color='Variable',
                    labels={'Tiempo': 'Tiempo (minutos)', 'Valor': 'Valor'},
                    title='Gráfica de potencias del ciclo',custom_data=['Indice'])
...
#
            ...
            if 'Rendimientos del compresor' in indicadores_termodinamicos and 'Gráfico de líneas' in tipo_grafico_diferido:
                ...
                if media.loc[i,'Rele compresor (on/off)']==0:
                    media.loc[i,'Rendimiento volumétrico (primera etapa de compresión) (%)']=0
                    media.loc[i,'Rendimiento isentrópico (primera y segunda etapa de compresión) (%)']=0
                    media.loc[i,'Rendimiento volumétrico (segunda estapa de compresión) (%)']=0
                ...
                fig_rendimientos= px.line(
                    ...
                    color='Variable',
                    labels={'Tiempo': 'Tiempo (minutos)', 'Valor': 'Valor'},
                    title='Gráfica de rendimientos del compresor',custom_data=['Indice'])
            ...
            if 'Caracterización de los evaporadores y gas-cooler' in indicadores_termodinamicos and 'Gráfico de líneas' in tipo_grafico_diferido:
                ...
                media.loc[i,'Salto térmico (salida - gass-cooler) (K)']=tabla_propiedades.loc['7','Temperatura (°C)']-media.loc[i,'Temperatura exterior (°C)']
                
                if media.loc[i,'Rele compresor (on/off)']==0:
                    media.loc[i,'Efectividad (evaporador 1) (%)']=0
                    media.loc[i,'Efectividad (evaporador 2) (%)']=0
                    media.loc[i,'Salto térmico (entrada - evaporador 1) (K)']=0
                    media.loc[i,'Salto térmico (salida - evaporador 1) (K)']=0
                    media.loc[i,'Salto térmico (entrada - evaporador 2) (K)']=0
                    media.loc[i,'Salto térmico (salida - evaporador 2) (K)']=0
                    media.loc[i,'Salto térmico (salida - gass-cooler) (K)']=0
            
                if media.loc[i,'Efectividad (evaporador 1) (%)']<0:
                    media.loc[i,'Efectividad (evaporador 1) (%)']=0
                if media.loc[i,'Efectividad (evaporador 2) (%)']<0:
                    media.loc[i,'Efectividad (evaporador 2) (%)']=0
                if media.loc[i,'Salto térmico (entrada - evaporador 1) (K)']<0:
                    media.loc[i,'Salto térmico (entrada - evaporador 1) (K)']=0
                if media.loc[i,'Salto térmico (salida - evaporador 1) (K)']<0:
                    media.loc[i,'Salto térmico (salida - evaporador 1) (K)']=0
                if media.loc[i,'Salto térmico (entrada - evaporador 2) (K)']<0:
                    media.loc[i,'Salto térmico (entrada - evaporador 2) (K)']=0
                if media.loc[i,'Salto térmico (salida - evaporador 2) (K)']<0:
                    media.loc[i,'Salto térmico (salida - evaporador 2) (K)']=0
                if media.loc[i,'Salto térmico (salida - gass-cooler) (K)']<0:
                    media.loc[i,'Salto térmico (salida - gass-cooler) (K)']=0
                ...
                fig_efectividad= px.line(
                    ...
                    color='Variable',
                    labels={'Tiempo': 'Tiempo (minutos)', 'Valor': 'Valor'},
                    title='Gráfica de efectividades de los evaporadores',custom_data=['Indice'])
                ...
                df_actual_saltos = media.loc[:i + 1,['Salto térmico (entrada - evaporador 1) (K)','Salto térmico (salida - evaporador 1) (K)','Salto térmico (entrada - evaporador 2) (K)','Salto térmico (salida - evaporador 2) (K)','Salto térmico (salida - gass-cooler) (K)','Tiempo','Indice']]
                ...
                fig_saltos= px.line(
                    ...
                    color='Variable',
                    labels={'Tiempo': 'Tiempo (minutos)', 'Valor': 'Valor'},
                    title='Gráfica de saltos térmicos de los evaporadores y del gas-cooler',custom_data=['Indice'])
            ...
            if 'Caracterización de los evaporadores y gas-cooler' in indicadores_termodinamicos and 'Tabla' in tipo_grafico_diferido:
                ...
                # Actualizar la tabla con los datos actuales
                hora_actual = media.loc[i]["Indice"]
                fila_actual = media.loc[i,['Efectividad (evaporador 1) (%)','Efectividad (evaporador 2) (%)','Salto térmico (entrada - evaporador 1) (K)','Salto térmico (salida - evaporador 1) (K)','Salto térmico (entrada - evaporador 2) (K)','Salto térmico (salida - evaporador 2) (K)','Salto térmico (salida - gass-cooler) (K)']].to_frame().T
                fila_actual.index = [hora_actual]
            ...
            if variables_ciclo and propiedades_ciclo and 'Gráfico de líneas' in tipo_grafico_diferido:
                ...
                propiedades_actual = list()
                propiedades_actual = tabla_propiedades.loc['1',:].tolist()+tabla_propiedades.loc['2',:].tolist()+tabla_propiedades.loc['3',:].tolist()+tabla_propiedades.loc['4s',:].tolist()+tabla_propiedades.loc['4',:].tolist()+tabla_propiedades.loc['5',:].tolist()+tabla_propiedades.loc['6s',:].tolist()+tabla_propiedades.loc['6',:].tolist()+tabla_propiedades.loc['7',:].tolist()+tabla_propiedades.loc['8',:].tolist()+tabla_propiedades.loc['9',:].tolist()+tabla_propiedades.loc['10',:].tolist()+tabla_propiedades.loc['11',:].tolist()+tabla_propiedades.loc['12',:].tolist()+tabla_propiedades.loc['13',:].tolist()+tabla_propiedades.loc['14',:].tolist()
                propiedades_actual = [0 if (not item) else item for item in propiedades_actual]
                tabla_acumulada.loc[i,'Temperatura (°C) - Punto 1':'Caudal másico (Kg/s) - Punto 14']=propiedades_actual

                #lista_propiedades_puntos=lista_propiedades_puntos+['Tiempo','Indice']
                #tabla_acumulada_elegida=tabla_acumulada[lista_propiedades_puntos].copy()
            
                df_actual_propiedades = tabla_acumulada.loc[:i + 1,lista_propiedades_puntos]
                df_melted_propiedades = df_actual_propiedades.melt(id_vars=['Tiempo','Indice'], var_name='Variable', value_name='Valor')

                fig_variables= px.line(
                    df_melted_propiedades,
                    x='Tiempo',
                    y='Valor',
                    color='Variable',
                    labels={'Tiempo': 'Tiempo (minutos)', 'Valor': 'Valor'},
                    title='Gráfica de propiedades termodinámicas de los puntos del ciclo',custom_data=['Indice'])
                fig_variables.update_traces(
                    hovertemplate="Tiempo: %{x}<br>Valor: %{y}<br>Hora: %{customdata[0]}<br>Variable: %{fullData.name}<extra></extra>"
                )
                fig_variables.update_layout(
                    ...
                    xaxis=dict(showgrid=True, gridcolor='lightgray',range=[0,df_ampliado_analogico['Tiempo'].max()],mirror=False,linecolor='black',tickfont=dict(color='black'),title=dict(text='Tiempo (minutos)', font=dict(color='black'))),
                    yaxis=dict(showgrid=True,gridcolor='lightgray',mirror=False,linecolor='black',tickfont=dict(color='black'),title=dict(text='Valor', font=dict(color='black'))))
                grafica_variables.plotly_chart(fig_variables)


            if variables_ciclo and propiedades_ciclo and 'Tabla' in tipo_grafico_diferido:

                if len(variables_ciclo)==1:
                    tam=20
                elif len(variables_ciclo)==2:
                    tam=108
                elif len(variables_ciclo)==3:
                    tam=140
                elif len(variables_ciclo)==4:
                    tam=177
                elif len(variables_ciclo)==5:
                    tam=212
                elif len(variables_ciclo)==6:
                    tam=246
                elif len(variables_ciclo)==7:
                    tam=280
                elif len(variables_ciclo)==8:
                    tam=315
                elif len(variables_ciclo)==9:
                    tam=350
                elif len(variables_ciclo)==10:
                    tam=387
                else:
                    tam=400
                ...
                tabla_propiedades.index.name = "Puntos"
                tabla_placeholder_propiedades_puntos.dataframe(tabla_propiedades.loc[variables_ciclo,propiedades_ciclo],height=tam,use_container_width=True,hide_index=False)
